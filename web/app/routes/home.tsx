...
const MemberList = () => {
  ...
  const openCreateModal = () => {
    ...
    setFormData({ fullName: '', age: '', sex: 'Male', maritalStatus: 'Single', children: 0, phone: '+255 ', regDate: '', expDate: '' });
    setIsModalOpen(true);
  };
            ...
            <table className="w-full text-left border-separate border-spacing-0">
              <thead className="sticky top-0 z-30 bg-white/95 backdrop-blur-md">
                <tr className="text-slate-400 text-[10px] uppercase tracking-[0.2em] font-black">
                  {['fullName', 'age', 'sex', 'maritalStatus', 'children', 'phone', 'regDate', 'expDate', 'isActive'].map((key) => (
              ...
              <tbody className="divide-y divide-slate-50">
                {sortedMembers.map((m) => (
                  <tr key={m.id} className="group hover:bg-blue-50/40 transition-all">
                    ...
                    <td className="px-8 py-5 text-slate-600 font-medium">{m.age}</td>
                    <td className="px-8 py-5 text-slate-600 font-medium">{m.sex}</td>
                    <td className="px-8 py-5 text-slate-500">{m.maritalStatus}</td>
                    <td className="px-8 py-5 text-center text-slate-600 font-bold">{m.children}</td>
                    <td className="px-8 py-5 font-mono text-sm text-slate-500">{m.phone}</td>
                    <td className="px-8 py-5 text-slate-500 text-sm whitespace-nowrap">{m.regDate}</td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md">
          <div className="bg-white rounded-[32px] w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-10 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1 text-blue-600">Phone (+255)</label>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Children</label>
                <input type="number" className="w-full px-5 py-4 bg-slate-100 border-none rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:bg-white outline-none font-medium transition-all"
                  value={formData.children} onChange={e => setFormData({...formData, children: e.target.value})} />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1 text-emerald-600">Reg. Date</label>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1 text-rose-600">Exp. Date</label>
                <input required type="date" className="w-full px-5 py-4 bg-slate-100 border-none rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:bg-white outline-none font-medium transition-all"
